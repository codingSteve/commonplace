...
Epictetus (\S \ref{EP_FOOL}) recommends that we need to gut things from our lives which take us away from the tricky but important business of improvement. We can be sure that nothing terrible will happen if we stop using social media. We can be confident that life will carry on if we ignore \say{the mind numbing spirit crushing game shows\footnote{Renton, Trainspotting}}. There are so many ways to spend our time, we need to be careful to choose well and that takes effort. Asking ourselves whether what we're doing right now is truly important to us (\S \ref{MA_DL}) is a vital habit to develop.
